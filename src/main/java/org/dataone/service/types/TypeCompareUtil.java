...
public class TypeCompareUtil {
    ...
    public static List<String> compareD1ServiceType(Map<String,Object> d1ServiceTypeMap, Boolean sortArrays) {
		...
		for (String label : d1ServiceTypeMap.keySet()) {
			Map<String,String> smdProps = TypeCompareUtil.getD1SubtypesListing("", d1ServiceTypeMap.get(label), sortArrays);
		...
		if (o == null) {
			...
			if (cl.getPackage().getName().startsWith("org.dataone.service.types")) {
				...
				for (Method m : methods) {
					...
					if ((m.getName().startsWith("get") && !m.getName().equals("getClass")
							...
							|| m.getName().equals("xmlValue")) {
						try {
							Object p = m.invoke(o, (Object[]) null);
							Map<String,String> subresults = getD1SubtypesListing(
									path + "/" + m.getName().replace("get", ""),
									p,
									sortArrays);
			...
			// otherwise, we are at a more basic datatype (String, etc)
				...
				if (o instanceof String) {
					...
					results.put(path, ((Date)o).toString());
				}
					...
					if ( !list.isEmpty() ) {
						...
						for (int j = 0; j < list.size(); j++) {
							results.putAll(getD1SubtypesListing( (path + "/" + j),
						             list.get(j),sortArrays));
						}
					} 
					else {
